...
export const Input = ({currentuser}) => {  
  ...
  return (
    <div className='input rounded-md bg-gradient-to-r from-yellow-500 via-pink-800 to-cyan-700  ' > 
      <input type="text" className=' ml-2 rounded-full h-11 mt-2 w-125 pl-3' value={text} onChange={(e)=>{settext(e.target.value)}} placeholder="Type something here ..."/> 
         ...
         <button  onClick={submit} className=' bg-black text-white p-0.5   px-1 rounded-md '>send</button>
         </div>
    </div>
  )
} 
